...
void main(void)
{
 ...
 //	CMT_init();  // CMTの初期化
	...
	/* Data Initialization */
	for(i=0;i<130;i++){
		ImageData[0][i] = 0;			/* カメラの値				*/
		ImageData[1][i] = 0;			/* カメラの値				*/
	...
	do{
		...
		if(cam_odd_even == 0)cam_odd_even = 1;
		else cam_odd_even = 0;
...
void ImageCapture(int linestart, int linestop){	 
	...
	for(i = LineStart; i < linestart; i++) {		
		...
		ImageData[cam_odd_even][i] = get_ad();
		TAOS_CLK_LOW;
	}
	...
	for(i = linestart; i <= linestop; i++) {				
		...
		ImageData[cam_odd_even][i] = get_ad();	// inputs data from camera (one pixel each time through loop) 
		TAOS_CLK_LOW;
		
		if(Max2 < (ImageData[0][i] + ImageData[1][i])){
			Max2 = (ImageData[0][i] + ImageData[1][i]);
			
			if(Max < (ImageData[0][i] + ImageData[1][i])){
				Max2 = Max;
				Max = (ImageData[0][i] + ImageData[1][i]);
			}
			
		}else if(Max < (ImageData[0][i] + ImageData[1][i])){
			Max2 = Max;
			Max = (ImageData[0][i] + ImageData[1][i]);
		}
		
		if(Min > (ImageData[0][i] + ImageData[1][i])){
			Min = (ImageData[0][i] + ImageData[1][i]);
		}
	...
	//if( Max > Line_Max - 200 ){//320 -150  250 目標値560用
		...
		//if(Max2 - Min < 230){//130 <-真っ白のときの明暗さで調整する
			...
			for(i = linestart ; i <= linestop; i++) {
				...
				if( (ImageData[0][i] + ImageData[1][i]) > Ave ){ //閾値以上	
					White++;			
					BinarizationData[i] = 1;
				}else{
