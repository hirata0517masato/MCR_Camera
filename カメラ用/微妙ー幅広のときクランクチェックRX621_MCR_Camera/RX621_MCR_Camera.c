...
////////////////////////////　　579行目付近も同時に調整すること
#define		Line_Max	560 //360 //460 //560 //760	// ライン白色MAX値の設定 
#define		Line_Min	200 //170 //200 //220 //250	//ラインの黒色はこの値より低くなるように設定する
...
void binarization(int linestart, int linestop)
{
	...
	if( Max2 > Line_Max - 250 ){//目標値560用
	//if( Max2 > Line_Max - 200 ){//目標値460用
	//if( Max2 > Line_Max - 100 ){//目標値360用
		/* 白が一直線のとき */
		if(Max2 - Min < 150){//130 <-真っ白のときの明暗さで調整する
			...
			if(mode == 0){//通常画角の時
				if(White > 24 ){//通常の幅より太い可能性がある場合（ハーフorクロス）
					...
					if(Min > Line_Min){//minが小さく無い場合はクロスラインとする
						...
						White = 126; //ログでどの条件で全白になったかわかるように127ではなく126にする
						for(i = linestart ; i <= linestop; i++) {
							BinarizationData[i] = 1;
						}
					}
