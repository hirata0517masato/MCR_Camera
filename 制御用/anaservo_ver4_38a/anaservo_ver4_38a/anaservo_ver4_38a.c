...
int			i_MOTOR_in_R_N=		6;//6		//内側モーター用パラメーター　後半	3	3
...
//クランク
int		    i_C_TOPSPEED	=		32;		//クランク(入)  25 33
...
//if(c_saka_cnt == 0){//２回目は絶対に検出しない　一時的な対策。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。		
		if(-15 < i && i < 15){
			if(c_mode == 0){//通常
				...
				if(angle_check() == 2 &&  (((l_EncoderTotal-l_startPoint_saka) >= 600) || ((c_saka_cnt%2 == 1) && ((l_EncoderTotal-l_startPoint_saka) >= 300))) && ((l_EncoderTotal-l_startPoint ) >= 500) ){//坂センサーチェック l_startPoint =クランク終了位置
					ul_cnt_saka++;
				}else{
					ul_cnt_saka = 0;
				}
