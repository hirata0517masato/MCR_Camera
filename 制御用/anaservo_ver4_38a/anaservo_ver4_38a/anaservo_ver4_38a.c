...
/////////////////////////////////////////////////////////////////////////////////////// 0:禁止 1と-1は同じ
//前半
int			i_SPEED_DOWN	=		4;		//角度によりi_TOPSPEEDを減速 カーブ前半 8 6
int			i_MOTOR_out_R	=		 1;		//外側モーター用パラメーター 1	-2
int			i_MOTOR_in_F	=		 2;		//内側モーター用パラメーター 	2 	1
int			i_MOTOR_in_R	=		 -2;		//内側モーター用パラメーター -2	-3
	
//後半
int			i_SPEED_DOWN_N=		6;		//角度によりi_TOPSPEEDを減速  カーブ後半 11 10
int			i_MOTOR_out_R_N=		3;		//外側モーター用パラメーター 後半	5	5
int			i_MOTOR_in_F_N=		7;		//内側モーター用パラメーター　後半	6	6
int			i_MOTOR_in_R_N=		4;		//内側モーター用パラメーター　後半	3	3
...
#define		Cu_FREE_time  		25		//カーブ終了時の後輪フリーの時間(msec）
...
#define			MOTOR_OUT_BASE_CH_Boost	45		//カーブ前半用　外側モーター用パラメーター クランク、ハーフの直後
...
#define			MOTOR_out_CH_R_Boost_min	 -20		//外側モーター用パラメーター 最小PWM	
#define			MOTOR_in_CH_F_Boost_min		 -10		//内側モーター用パラメーター　最小PWM
#define			MOTOR_in_CH_R_Boost_min		 -20		//内側モーター用パラメーター　最小PWM
...
int		    i_H_TOPSPEED2	=		45;		//ハーフ(斜め) ブレーキがかからないように値を高くする
#endif
...
//if(c_saka_cnt == 0){//２回目は絶対に検出しない　一時的な対策。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。		
		if(-15 < i && i < 15){
			if(c_mode == 0){//通常
				...
				if(angle_check() == 2 &&  (((l_EncoderTotal-l_startPoint_saka) >= 600) || ((c_saka_cnt%2 == 1) && ((l_EncoderTotal-l_startPoint_saka) >= 300))) && ((l_EncoderTotal-l_startPoint ) >= 1000) ){//坂センサーチェック l_startPoint =クランク終了位置
					ul_cnt_saka++;
				}else{
					ul_cnt_saka = 0;
				}
        ...
        if(i_date_f_mode == 0 || c_c_cut == 0){//通常
			...
			if(c_c_cut_short == 1){//距離が短いとき
				...
				//	l_startPoint += 2; //手前で曲がりすぎているため距離を少しのばす
					...
					if(i < 40)i_SetAngle = 90;
					else i_SetAngle = 60;
				
					motor2_f( 30,   -10 ); //85 30        
	        		motor2_r( -5,   -25 );  //25 0
			
				}else if((l_EncoderTotal-l_startPoint ) >= 50){
				
					if(i < 30)i_SetAngle = 60;
					else i_SetAngle = 45;
				
					motor2_f( 35, -10 ); //90 35        
	        		motor2_r( 0,  -20 );   //35 0
        ...
        if(i_date_f_mode == 0 || c_c_cut == 0){//通常
			...
			if(c_c_cut_short == 1){//距離が短いとき
				...
				if((i_Center < 0 && (l_EncoderTotal-l_startPoint ) >= 150)){
					...
					if(i > -35)i_SetAngle = -85;
					else i_SetAngle = -55;
				
					motor2_f( -10,   30 ); //30 85        
	        		motor2_r( -5,   -5 ); //0 25
			
				}else if((l_EncoderTotal-l_startPoint ) >= 50){
					if(i > -25)i_SetAngle = -55;
					else i_SetAngle = -40;
				
					motor2_f( -10,   35 ); //35 90       
	        		motor2_r( -15,  0 );   //0 35
...
unsigned char check_halfline( void )
{
	...
	if(i_Wide > 44 && i_Wide < 50){
		if(i_Center < -12){//センター左寄り
			uc_ret = 1;
			
		}else if(i_Center > 12){//センター右寄り
			uc_ret = 2;
...
int angle_check(){
	...
	if(200 < i_angle_y && i_angle_y < 400){
		if(i_angle_x <= 170)return 2;//上 170
