...
int  		i_Center_offset_Angle = -4;	/*この値につき１ＩＮ側に寄せる	正：IN　負：OUT		*/
...
//斜面(上り)
#define		    TOPSPEED2			46		//直線(坂）30 33
...
#define			MOTOR_out_CH_R_Boost_min	 -15//-20		//外側モーター用パラメーター 最小PWM	
#define			MOTOR_in_CH_F_Boost_min		 -10 //-10		//内側モーター用パラメーター　最小PWM
#define			MOTOR_in_CH_R_Boost_min		 -15//-20		//内側モーター用パラメーター　最小PWM
...
char		c_c_cut  	 	 =	  1;	//再生走行時であっても 0= 再生しない 1= 再生する
        ...
        if(i_date_f_mode == 0 || c_c_cut == 0){//通常
			...
			if(c_c_short_mode == 1){//距離が短いとき
				...
				//	l_startPoint += 2; //手前で曲がりすぎているため距離を少しのばす
					...
					if(i < 50)i_SetAngle = 120;
					else i_SetAngle = 60;//50;
				
					motor2_f( 35,  -15 ); //85 30        
	        		motor2_r( -10,   -30 );  //25 0
					...
					motor2_f( 40, -15 ); //90 35        
	        		motor2_r( -5,  -30 );   //35 0
					
				}else{
					i_SetAngle = 25;
					motor2_f( 40,  -10 );  //90 40       
	        		motor2_r( 0,   -30 );   //40  0	
				} 
				...
				//	l_startPoint += 2; //手前で曲がりすぎているため距離を少しのばす
					...
					else if(i > 23)i_SetAngle = 5;
					else i_SetAngle = 20; //35
        ...
        if(i_date_f_mode == 0 || c_c_cut == 0){//通常
			...
			if(c_c_short_mode == 1){//距離が短いとき
				...
				if((i_Center < 0 && (l_EncoderTotal-l_startPoint ) >= 150)){
					...
					motor2_f( -15,   40 ); //35 90       
	        		motor2_r( -30,  -5 );   //0 35
					
				}else{
					i_SetAngle = -25;
					motor2_f( -10,   40 );  //40 90    
	        		motor2_r( -30,   0 );   // 0 40
			...
			if((i_pattern == 11 || i_pattern == 10) && (((c_mode == 0) &&(-i_Cu_Angle < a && a < i_Cu_Angle)) || (((c_mode != 0) || ( (l_EncoderTotal - l_startPoint_saka) < 1500 ) ) &&(-(i_Cu_Angle_saka) < a && a < i_Cu_Angle_saka)) ) ){
				...
				if(c_mode == 0){
					...
					if(l_straight_EncoderTotal > 300){
						if(i_date_f_buff_int[i_date_f_num] - 300 < l_straight_EncoderTotal)si_flag = 1;//記録した直線を走った
					}
				}else{
					//if(i_date_f_buff_int[i_date_f_num] - date_f_brake - 500 < l_straight_EncoderTotal)si_flag = 1;//記録した直線を走った
					
					if(l_straight_EncoderTotal > 300){
						if(i_date_f_buff_int[i_date_f_num] - 300 - 600 < l_straight_EncoderTotal)si_flag = 1;//記録した直線を走った
					}
				}
				si_flag20 = 0;
