...
#define 	MAXTIME 			1400 //1100	  				/* 最大走行時間 (0.01秒)  1200 = 12s     1250     */
...
int				i_Cu_Angle_saka	=	40;		//カーブ判定に使用 正数限定 坂用
...
long			l_startPoint_saka_ue_fin = 0;/* 距離計測用スタート地点 坂？ */
...
**************************************************************************************************/
//オフセット
int  		i_Center_offset_MAX = 0;		/*カーブ時カメラセンターを移動＝寄せる 最小値 0 	*/
int  		i_Center_offset_Angle = -100;	/*この値につき１ＩＮ側に寄せる	正：IN　負：OUT		*/
...
//斜面(上り)
#define		    TOPSPEED2			47		//直線(坂）30 33
...
//コース記憶用
#define			MOTOR_OUT_BASE_CH_Boost	 90//45		//カーブ前半用　外側モーター用パラメーター クランク、ハーフの直後
...
//クランク
int		    i_C_TOPSPEED	=		30;		//クランク(入)  25 33
int		    i_C_TOPSPEED2	=		50;		//クランク(出)	40
int			i_C_short_len =		540;	//この距離未満はショート、以上はロング ※再生走行用と合わせたほうがいいかも
#define		C_TOPSPEED_SHORT	4		//通常のみ (i_Encoder10 > C_TOPSPEED + C_TOPSPEED_SHORT )のとき　＝減速できていない場合はショート
#define		C_TOPSPEED_SHORT_NG	4		//通常のみ (i_Encoder10 < C_TOPSPEED - C_TOPSPEED_SHORT_NG )のとき　＝距離はショートでも速度が遅いときはロング


char		c_c_cut  	 	 =	  0;	//再生走行時であっても 0= 再生しない 1= 再生する

int			i_date_f_brake_c	=	550;	//再生走行時のブレーキ使用可能距離(mm) クランク用 600
int			i_date_f_shortcat_c =	230;	//再生走行時のショートカット距離(mm) クランク用 210

int 		i_C_TOPSPEED4 = 		48;		//再生走行時のブレーキ前
int		    i_C_TOPSPEED3	=		40;		//再生走行用クランク(入)  25 33 
int			i_C_short_len_Boost	  =	540;  	//再生走行時 この距離未満はショート、以上はロング
...
void main( void )
{
	...
	if( (dipsw_get() & 0x01) == 0x01 ) {
		...
		i_TOPSPEED = i_TOPSPEED_B;//コース記憶用に置き換え
...
//}		
		...
		if(-20 < i && i < 20){
			if(c_mode == 0){//坂中でなければ
				if(l_EncoderTotal > 200 && (l_startPoint_saka == 0 || (((l_EncoderTotal-l_startPoint_saka) >= 500) && ((l_EncoderTotal-l_startPoint_saka_ue_fin) >= 2000)) ) && (l_startPoint == 0 || (l_EncoderTotal-l_startPoint ) >= 150) && (l_startPoint_curve == 0 || (l_EncoderTotal-l_startPoint_curve) >= 0)){//ゲートに反応しないように && 坂終了から少しの間は無視 && クランク、ハーフ終了後少し無視 && カーブ直後は無視
					...
					if(i_date_f_mode == 0){
						...
						if( (check_crossline() || check_halfline() != 0 ) || check_wideline() == 1){ 
			...
			if(ul_cnt_straight_time_1ms <= Cu_BRAKE_time && (l_EncoderTotal-l_startPoint ) >= 100 && (l_EncoderTotal-l_startPoint_saka) >= 100){//カーブ進入時のブレーキ
				...
				if((ul_cnt_curve_time_1ms <= Cu_N_time) || (c_mode == 1) || (i > 95) ){//カーブ前半 || 坂モード || 曲げすぎ || 
					...
					if(c_ch_boost_on == 1){//ショートカットクランク直後
						...
						if(4 < (i_Center + i_Center_offset)){//外によりすぎ
							 f /= 2;
			...
			if(ul_cnt_straight_time_1ms <= Cu_BRAKE_time && (l_EncoderTotal-l_startPoint ) >= 100 && (l_EncoderTotal-l_startPoint_saka) >= 100){//カーブ進入時のブレーキ
				...
				if((ul_cnt_curve_time_1ms <= Cu_N_time) || (c_mode == 1) || (i < -95) ){//カーブ前半 || 坂モード || 大曲
					...
					if(c_ch_boost_on == 1){//ショートカットクランク直後
						...
						if((i_Center + i_Center_offset) < -4 ){//外によりすぎ
							f /= 2;
							if(r < -30) r = -30;//-10
							if(or < -5) or = -5;	//0
    ...
    case 22:	
		...
		if( (l_EncoderTotal-l_startPoint ) >= 250 ) {
			...
			if((i_date_f_mode == 0 && check_halfline_forC() == 2) || ((c_c_cut == 1 && i_date_f_mode != 0) && i_date_f_buff_ch_int[i_date_f_num_ch-1] == 31  && ((i_date_f_buff_ch_int[i_date_f_num_ch] - i_date_f_shortcat_c )< (l_EncoderTotal - l_EncoderTotal_ch)))
				...
				//ショートorロングの判定
				if(c_c_cut == 1 && i_date_f_mode != 0){//コース記憶モードでは距離と速度が安定している前提なので計測距離から判定
					
					if(i_C_short_len_Boost > i_date_f_buff_ch_int[i_date_f_num_ch]){
						c_c_short_mode = 1;
					...
					c_c_short_mode = 1;	//すべて距離が 500mm として調整する
        ...
        if(i_date_f_mode == 0 || c_c_cut == 0){//通常
			...
			if(c_c_short_mode == 1){//距離が短いとき
				...
				//	l_startPoint += 2; //手前で曲がりすぎているため距離を少しのばす
					...
					if(i < 90)i_SetAngle = 150;
					else i_SetAngle = 100;//80
					...
					if(i < 80)i_SetAngle = 120;
					else i_SetAngle = 90;//50;
				
					motor2_f( 30,  -15 ); //85 30        
	        		motor2_r( -10,   -30 );  //25 0
			
				}else if((l_EncoderTotal-l_startPoint ) >= 50){
				
					if(i < 50)i_SetAngle = 100;
					else i_SetAngle = 60;
				
					motor2_f( 30, -15 ); //90 35        
	        		motor2_r( -5,  -30 );   //35 0
					
				}else{
					if(i < 25)i_SetAngle = 60;
					i_SetAngle = 30;
					
					motor2_f( 35,  -10 );  //90 40       
	        		motor2_r( 0,   -30 );   //40  0	
				} 
        ...
        if(i_date_f_mode == 0 || c_c_cut == 0){//通常
			...
			if(c_c_short_mode == 1){//距離が短いとき
				...
				if((i_Center < 0 && (l_EncoderTotal-l_startPoint ) >= 150)){
					...
					motor2_f( -15,   30 ); //35 90       
	        		motor2_r( -30,  -5 );   //0 35
					
				}else{
					if(i > -25)i_SetAngle = -60;
					i_SetAngle = -30;
					motor2_f( -10,   35 );  //40 90    
	        		motor2_r( -30,   0 );   // 0 40
...
unsigned char check_crossline( void )
{
	...
	uc_ret = 0;
	if( (i_Wide > 60) || ((i_Wide >= 40) && (-8 < i_Center ) && (i_Center < 8)) || ((i_Wide >= 28) && (i_Center > -4) && (i_Center < 4)) ){
